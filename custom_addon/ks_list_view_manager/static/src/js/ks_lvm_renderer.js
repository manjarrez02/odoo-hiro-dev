...
patch(ListRenderer.prototype, "ks_lvm_renderer", {
    ...
    async _mounted() {
        if ($(document.querySelector('.modal-content')).length ===0){
            var self = this;
            ...
            if(document.querySelector(".o_list_controller.o_list_actions_header") && $(document.querySelector('.modal-content')).length ===0){
    ...
    ks_textBox: function (node) {
        ...
        if (!this.ks_is_lines){
            if (node.type === "field") {
                if (self.props.list.fields[node.name].store === true &&  node.name != 'sequence' && !(self.props.list.fields[node.name].type === "one2many")) {
