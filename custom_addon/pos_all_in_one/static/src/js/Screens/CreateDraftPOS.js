odoo.define('pos_all_in_one.CreateDraftPOS', function(require) {
	...
	ProductScreen.addControlButton({
		...
		condition: function() {
			if(!this.env.pos.config.module_pos_hr && this.env.pos.user.is_allow_draft_order){
			...
			if(this.env.pos.config.module_pos_hr && this.env.pos.cashier.is_allow_draft_order){
				if(this.env.pos.config.allow_partical_payment){
					return true
				}else{
					return true
