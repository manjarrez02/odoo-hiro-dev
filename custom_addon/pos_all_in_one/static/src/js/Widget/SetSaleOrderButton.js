odoo.define('pos_sale.SetSaleOrderButton', function(require) {
	...
	class SetSaleOrderButton extends PosComponent {
			setup() {
				super.setup();
				var self = this;
				// if(!self.env.pos.config.module_pos_hr && self.env.pos.user.is_allow_quotation_order){
				// 		useListener('click', this.onClick);
				// }
				// if(self.env.pos.config.module_pos_hr && self.env.pos.cashier.is_allow_quotation_order){
				// }
				useListener('click', this.onClick);
			}
			get currentOrder() {
	...
	ProductScreen.addControlButton({
		...
		condition: function () {
            if(this.env.pos.config.module_pos_hr && this.env.pos.cashier.is_allow_quotation_order){
                return true;
            }else if(!this.env.pos.config.module_pos_hr && this.env.pos.user.is_allow_quotation_order){
                return true;
            }
        },
	});
