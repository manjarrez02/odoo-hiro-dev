odoo.define('pos_all_in_one.CreatePaymentButtonWidget', function(require) {
	...
	ProductScreen.addControlButton({
		...
		condition: function() {
			if(!this.env.pos.config.module_pos_hr && this.env.pos.user.is_allow_payment_action){
				if(this.env.pos.config.allow_pos_payment){
					return true
				}else{
					return true
				}
			}
			if(this.env.pos.config.module_pos_hr && this.env.pos.cashier.is_allow_payment_action){
				if(this.env.pos.config.allow_pos_payment){
					return true
				}else{
					return true
