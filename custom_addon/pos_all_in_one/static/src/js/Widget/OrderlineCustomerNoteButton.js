odoo.define('point_of_sale.OrderlineCustomerNoteButton', function(require) {
    ...
    class OrderlineCustomerNoteButton extends PosComponent {
        setup() {
            super.setup();
            var self = this;
            // if(!self.env.pos.config.module_pos_hr && self.env.pos.user.is_allow_customer_note){
            //     useListener('click', this.onClick);
            // }
            // if(self.env.pos.config.module_pos_hr && self.env.pos.cashier.is_allow_customer_note){
            // }
            useListener('click', this.onClick);
        }
        async onClick() {
    ...
    ProductScreen.addControlButton({
        ...
        condition: function () {
            if(this.env.pos.config.module_pos_hr && this.env.pos.cashier.is_allow_customer_note){
                return true;
            }else if(!this.env.pos.config.module_pos_hr && this.env.pos.user.is_allow_customer_note){
                return true;
            }
        },
    });
