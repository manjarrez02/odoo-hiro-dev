...
class POSConfigInherit(models.Model):
	...
	allow_to_pay_partical_order = fields.Boolean('Allow To Pay Partial Order')
...
class ResConfigSettings(models.TransientModel):
	...
	allow_to_pay_partical_order = fields.Boolean(related='pos_config_id.allow_to_pay_partical_order',readonly=False)
...
class PosOrderInherit(models.Model):
	...
	def _prepare_cust_disc_invoice_line(self):
		return {
			'product_id': self.config_id.custdisc_product_id.id,
			'quantity': 1,
			'discount': 0,
			'price_unit': -self.pos_customer_discount,
			'name':  self.config_id.custdisc_product_id.display_name,
			'tax_ids': [(6, 0,[])],
			'product_uom_id': self.config_id.custdisc_product_id.uom_id.id,
		}

	def _prepare_invoice_vals(self):
		res = super(PosOrderInherit, self)._prepare_invoice_vals()
		if self.pos_customer_discount > 0 :
			sc_line = self._prepare_cust_disc_invoice_line()
	...
	cust_saleper_id = fields.Many2one('res.users', string='Sales persons')
	cust_saleper_limit_id = fields.Many2one('res.users', string='Manager for credit limit')
	cust_saleper_due_id = fields.Many2one('res.users', string='Manager for Balance Due')
	pos_customer_discount = fields.Float(string="Customer Discount")
	...
	def _order_fields(self, ui_order):
		...
		res['cust_saleper_id'] = ui_order.get('cust_salep_id',False)
		res['cust_saleper_due_id'] = ui_order.get('cust_saleper_due_id',False)
		res['cust_saleper_limit_id'] = ui_order.get('cust_saleper_limit_id',False)
		res['pos_customer_discount'] = ui_order.get('pos_customer_discount',0.0)
		return res
	...
	def _process_order(self, order, draft, existing_order):
		...
		if create_invoice:
			if pos_order.session_id.config_id.allow_partical_payment and pos_order.is_partial:
				if order['to_invoice'] and pos_order.state == 'draft':
					pos_order.action_pos_order_invoice()
			else:
				if order['to_invoice'] and pos_order.state == 'paid':
					pos_order.action_pos_order_invoice()
	...
	def _accumulate_amounts(self, data):
		...
		for order in order_ids:
			...
			if not order_is_invoiced:
				...
				if order.pos_customer_discount > 0:
					prod = self.config_id.custdisc_product_id
					service_prod_income_account = prod.with_context(
						with_company=order.company_id.id).property_account_income_id or prod.categ_id.with_context(
						with_company=order.company_id.id).property_account_income_categ_id
					...
					sales[sale_key1] = self._update_amounts(sales[sale_key1], {'amount': -(order.pos_customer_discount)},
															order.date_order)
