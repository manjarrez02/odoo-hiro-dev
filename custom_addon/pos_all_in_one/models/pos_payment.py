...
class pos_create_customer_payment(models.Model):
	...
	def create_customer_payment_inv(self, partner_id, journal, amount, invoice, note, session_id):
		payment_object = self.env['account.payment']
#		partner_object = self.env['res.partner']
		...
		pass_data = {'payment_type':a.name, 'payment_journal':a.journal_id.name, 'partner':a.partner_id.name}
		return pass_data
...
class AccountInvoice(models.Model):
	...
	def action_post(self):
		...
		for r in self:
			r.partner_id.update_partner_credit(r.amount_total)
		return res
