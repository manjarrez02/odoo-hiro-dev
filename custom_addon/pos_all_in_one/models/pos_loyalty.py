...
class res_partner(models.Model):
	...
	loyalty_points1 = fields.Float(string='Loyalty Points',compute='_compute_loyalty_points1',store=True)
	...
	bi_not_allow_partial_payment = fields.Boolean(string='Allowed For Partial Payment')
	bi_total_credit_amount = fields.Float(string='Credit Amount ')
	...
	def update_partner_credit(self, amount):
		if self.allow_over_limit:			
			self.update({'bi_total_credit_amount': float(self.bi_total_credit_amount) + float(amount)})
			

	def update_partner_credit_amount(self, amount):
		if self.allow_over_limit:			
			#self.update({'bi_total_credit_amount': float(self.bi_total_credit_amount) - float(amount)})
			account_move = self.env['account.move'].search([('partner_id', '=', self.id),('state', '=', 'posted'),('amount_residual', '>', 0)])
			due_amount = round(sum(account_move.mapped('amount_residual')),2)
			self.update({'bi_total_credit_amount': float(due_amount)})

	@api.depends('loyalty_history_ids','loyalty_history_ids.points','loyalty_history_ids.transaction_type')
	def _compute_loyalty_points1(self):
		for rec in self:
	...
	def action_view_loyalty_points1(self):
		self.ensure_one()
...
class pos_order(models.Model):
	...
	def check_change_credit(self, amount, journal, session_id):
		...
		currency_id = payment_method.journal_id.currency_id if payment_method.journal_id.currency_id else False
	...
	def create_from_ui(self, orders, draft=False):
		...
		return self.env['pos.order'].search_read(domain=[('id', 'in', order_ids)],
												 fields=['id', 'name', 'pos_reference', 'account_move'], load=False)
