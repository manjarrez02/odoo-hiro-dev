...
class ClosedSessionReportCustom(models.AbstractModel):
    _inherit = 'report.pos_all_in_one.report_closed_session'
    ...
    def get_sale_details(self, sessions=False, company=False):
        ...
        for i in sessions:
            ...
            stop_local = fields.Datetime.context_timestamp(self, i.stop_at)
            
            # Formatea el nombre de la sesión con fechas de inicio y fin
            session_info = f"{i.name} ({start_local.strftime('%d/%m/%Y %H:%M:%S')} → {stop_local.strftime('%d/%m/%Y %H:%M:%S')})"
            sessions_name.append(session_info)

        num_sessions = '\n'.join(map(str,sessions_name) )
