...
class ZkMachine(models.Model):
    ...
    attendance_type = fields.Selection([('0', 'Undefined'),
                                        ('1', 'Finger'),
                                        ('15', 'Face'),
                                        ('25', 'Palm'),
                                        ('2','Type_2'),
...
class HrAttendanceExtended(models.Model):
    ...
    break_duration = fields.Float(string="Break Duration (hrs)", compute="_compute_break_duration", store=True)

    @api.depends('break_out', 'break_in')
    def _compute_break_duration(self):
        for record in self:
            if record.break_out and record.break_in:
                delta = record.break_in - record.break_out
                record.break_duration = delta.total_seconds() / 3600.0
            else:
                record.break_duration = 0.0


    @api.depends('check_in', 'check_out', 'break_out', 'break_in')
    def _compute_worked_hours(self):
        for attendance in self:
            if attendance.check_out and attendance.check_in:
                delta = attendance.check_out - attendance.check_in
                if attendance.break_out and attendance.break_in:
                    delta = delta - (attendance.break_in - attendance.break_out)
